...
if (keyring::keyring_is_locked(keyring = config$keyringName)) {
  keyring::keyring_unlock(keyring = config$keyringName, password = Sys.getenv("STRATEGUS_KEYRING_PASSWORD"))
}
...
tcis <- list(
  ...
  list(
    targetId = 12676, # [EPI_555] New-Users of a First-Line Thiazide or Thiazide-like Diuretics (THZ)
    comparatorId = 12672, # [EPI_555] New-Users of a First-Line Angiotensin Converting Enzyme Inhibitors (ACEi)
    indicationId = NULL, # [EPI_555] Hypertensive Disorder
...
outcomes <- tibble(
  cohortId = c(12677,12678,12679), # [EPI_555] New-Users of a First-Line Angiotensin Converting Enzyme Inhibitors (ACEi)
  cleanWindow = c(365)
)
negativeConceptSetId <- 5263
timeAtRisks <- tibble(
...
httr::set_config(httr::config(ssl_verifypeer = FALSE)) # JNJ specific requirement when using ROhdsiWebApi
...
ParallelLogger::saveSettingsToJson(analysisSpecifications, file.path(config$rootFolder, config$studySpecificationFileName))
